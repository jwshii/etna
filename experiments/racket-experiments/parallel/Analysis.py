...
def plot_data(df: pd.DataFrame, figures: str, limit_type: str, prefix: str, case: str, show_names: bool = False):
    ...
    # - x-axis is the number of tasks solved within a search-time limit
    # - colors denote the search-time limit, we create a gradient of colors for each strategy
    ...
    suffix = "search-time" if limit_type == "search-time" else "inputs"
...
if __name__ == "__main__":
    ...
    for case in ["BST", "RBT", "STLC"]:
        plot_data(df, images_path, "search-time", "task_bucket", case, show_names=False)
        plot_data(df, images_path, "search-time", "task_bucket_named", case, show_names=True)
