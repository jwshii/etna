...
def draw_scatter_plot(data, output_file):
    ...
    tick_height = (height - 2 * padding) / 40
    for i in range(1, 40):
        # i = i 
    ...
    for i, ((mutant, property), values) in enumerate(data.items()):
        ...
        for j, (strategy, sizes) in enumerate(values.items()):
            ...
            shrinkage_average = sum(sizes["Shrinkage"]) / len(sizes["Shrinkage"])
            avgs[strategy] = (sizeavg, shrinked_sizeavg, shrinkage_average)
...
def sort_by_rackcheck_shrinked_size(x):
    ...
    if len(filtered) == 0:
        return 0
    # print(filtered)
    shrinkage = list(map(lambda x: x[0] - x[1], filtered))
    # print(shrinkage)
    x[1]["RackcheckBespoke"]["Shrinkage"] = shrinkage
    x[1]["RackcheckBespoke"]["ShrinkedSize"] = list(map(lambda x: x[1], filtered))
    x[1]["RackcheckBespoke"]["Size"] = list(map(lambda x: x[0], filtered))

    zipped = zip(x[1]["ProplangBespoke"]["Size"], x[1]["ProplangBespoke"]["ShrinkedSize"])
    filtered = list(filter(lambda x: x[1] != -1, zipped))
    pshrinkage = list(map(lambda x: x[0] / x[1], filtered))
    x[1]["ProplangBespoke"]["Shrinkage"] = pshrinkage
    x[1]["ProplangBespoke"]["ShrinkedSize"] = list(map(lambda x: x[1], filtered))
    x[1]["ProplangBespoke"]["Size"] = list(map(lambda x: x[0], filtered))
