...
def collect(results: str):
    ...
    for workload in tool.all_workloads():
        if workload.name not in ['BSTProplang', 'RBTProplang']:
            continue

        tool._preprocess(workload)

        variable = next(filter(lambda v: v.name == "PropLangVersion", tool.all_variables(workload)))

        tool.update_variable(workload, variable, 1)
