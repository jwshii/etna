...
type ('a, 'b) tree =
  | E
  | T of color * ('a, 'b) tree * 'a * 'b * ('a, 'b) tree
...
let redden (t: ('a, 'b) tree) : ('a, 'b) tree option =
  match t with
  | T (B, l, k, v, r) -> Some (T (R, l, k, v, r))
  | _ -> None


let balance (col: color) (tl: ('a, 'b) tree) (k: 'a) (v: 'b) (tr: ('a, 'b) tree) : ('a, 'b) tree =
...
let rec insert (k: 'a) (v: 'b) (t: ('a, 'b) tree) : ('a, 'b) tree =
    ...
    in blacken (ins k v t)


let balLeft (tl: ('a, 'b) tree) (k: 'a) (v: 'b) (tr: ('a, 'b) tree) : ('a, 'b) tree option =
  ...
  (*! *)
    redden c >>=
    (fun c' -> Some (T (R, T (B, bl, x, vx, a), y, vy, (balance B b z vz c'))))
  (*!! miscolor_balLeft *)
  (*!
...
let balRight (tl: ('a, 'b) tree) (k: 'a) (v: 'b) (tr: ('a, 'b) tree) : ('a, 'b) tree option =
    match tl, k, v, tr with
    ...
    (*! *)
        redden a >>=
        (fun a' -> Some (T (R, balance B a' x vx b, y, vy, T (B, c, z, vz, bl))))
    (*!! miscolor_balRight *)
    (*!
...
let rec _join (t1: ('a, 'b) tree) (t2: ('a, 'b) tree) (f: int) : ('a, 'b) tree option =
    match f with
    | 0 -> None
    | f -> let f' = f - 1 in
        match t1, t2 with
...
let rec del (x: 'a) (s: ('a, 'b) tree) (f: int) : ('a, 'b) tree option =
    ...
    | f -> let f' = f - 1 in
        ...
        | T (_, a, y, vy, b) ->
            (*! *)
            if x < y then
                delLeft x a y vy b f'
            else if y < x then
                delRight x a y vy b f
            else
                join a b
            (*!! delete_4 *)
            (*!
            ...
            *)
and delLeft (x: 'a) (dl: ('a, 'b) tree) (dy: 'a) (dvy: 'b) (dr: ('a, 'b) tree) (f: int): ('a, 'b) tree option =
    match f with
    | 0 -> None
    | f -> let f' = f - 1 in
        match dl, dy, dvy, dr with
        | T (B, al, ax, avx, ar), y, vy, b ->
            del x (T (B, al, ax, avx, ar)) f' >>=
            (fun t' -> balLeft t' y vy b)
        | a, y, vy, b ->
            del x a f' >>=
            (fun t' -> Some (T (R, t', y, vy, b)))
and delRight (x: 'a) (dl: ('a, 'b) tree) (dy: 'a) (dvy: 'b) (dr: ('a, 'b) tree) (f: int): ('a, 'b) tree option =
    match f with
    | 0 -> None
    | f -> let f' = f - 1 in
        match dl, dy, dvy, dr with
        | a, y, vy, T (B, bl, bx, bvx, br) ->
            del x (T (B, bl, bx, bvx, br)) f' >>=
            (fun t' -> balRight a y vy t')
        | a, y, vy, b ->
            del x b f' >>=
            (fun t' -> Some (T (R, a, y, vy, t')))
