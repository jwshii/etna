...
(*
Fixpoint liftMemSState (st1 st2 : SState) (lm : list (@Variation mem)) :=
  match st1, st2 with
      ...
      match lm with
        ...
        | cons (Var lab m1' m2') t =>
          cons (Var lab (St m1' p1 s1 regs1 pc1) (St m2' p2 s2 regs2 pc2))
               (liftMemSState st1 st2 t)
      end
  end.
*)

Fixpoint liftStackSState (st1 st2 : SState) (ls : list (@Variation Stack)) :=
  match st1, st2 with
      ...
      match ls with
        ...
        | cons (Var lab s1' s2') t =>
          cons (Var lab (St im1 m1 s1' regs1 pc1) (St im2 m2 s2' regs2 pc2))
               (liftStackSState st1 st2 t)
      end
  end.

(*
Definition shrinkSStateMemory (v : @Variation SState) :=
  let '(Var lab st1 st2) := v   in
  let '(St _ m  p  s  regs1 pc ) := st1 in
  let '(St _ m' p' s' regs2 pc') := st2 in
  liftMemSState st1 st2 (shrinkV (Var lab m m')).
*)

Definition shrinkSStateStack (v : @Variation SState) :=
  let '(Var lab st1 st2) := v in
  let '(St _ _ s  _ _) := st1 in
  let '(St _ _ s' _ _) := st2 in
  liftStackSState st1 st2 (shrinkV (Var lab s s')).

Definition shrinkSState x :=
  (* shrinkSStateMemory x ++ *) shrinkSStateStack x.
...
Definition removeReg (st : SState) (r : regId) :=
...
Fixpoint liftRegsSState (st1 st2 : SState) (ls : list (@Variation regSet)) :=
  match st1, st2 with
    | St im1 m1 s1 _ pc1, St im2 m2 s2 _ pc2 =>
      match ls with
        ...
        | cons (Var lab regs1' regs2') t =>
          cons (Var lab (St im1 m1 s1 regs1' pc1) (St im2 m2 s2 regs2' pc2))
               (liftRegsSState st1 st2 t)
      end
  end.

Definition shrinkVRegs (v : @Variation SState) :=
...
(*    let regsNo := List.length regs - 1 in
    ...
    let sts' := map (removeReg st') allRegs in
    map (fun x : (SState * SState) =>
           let (st1,st2) := x in Var l st1 st2) (combine sts sts') ++*)
    (liftRegsSState st st' (shrinkVRegContents l nil nil regs regs')).

Definition shrinkVSState (x : @Variation SState) :=
  shrinkSState x
(*  ++ concat (map shrinkSStateMemory (shrinkSStateMemory x))
  ++ concat (map shrinkSStateMemory (shrinkSStateStack x))
  ++ concat (map shrinkSStateStack  (shrinkSStateMemory x))*)
  ++ shrinkVRegs x.
(*
