...
Definition max_frame_size := 10%Z.

(* Could also vary this - but no need *)
Definition code_len := 30.
...
(* LEO: For LLNI, stack should start empty. *)
Definition smart_gen_stack (inf : Info) : G Stack :=
  ret (ST nil).
(*  freq_ (pure (ST nil))
            [:: (1, pure (ST nil));
...
Definition ainstrSSNI (st : SState) : G Instr :=
  ...
  freq_ (pure Nop) [::
    ...
    (* Alloc *)
    (10 * onNonEmpty num 1 * onNonEmpty lab 1,
     liftGen3 Alloc (elems_ Z0 num) (elems_ Z0 lab) genRegPtr);
    (* Load *)
    (onNonEmpty dptr 10, liftGen2 Load (elems_ Z0 dptr) genRegPtr);
    (* Store *)
    (onNonEmpty dptr 10, liftGen2 Store (elems_ Z0 dptr) genRegPtr);
    (* Write *)
    (onNonEmpty dptr 10, liftGen2 Write (elems_ Z0 dptr) genRegPtr);
    (* Jump *)
    (onNonEmpty cptr 10, liftGen Jump (elems_ Z0 cptr));
    (* BNZ *)
    (onNonEmpty num 10,
      (* BNZ target should be more likely forward,
         chance for backward.*)
      let Zl := Zlength im in
      let gen_offset := 
        if Z.ltb (Zplus pc_i (1%Z)) Zl then
          oneOf_ (ret 0%Z)
                (cons (choose (2%Z, Zminus Zl pc_i))
                (cons (choose (Zminus (0%Z) pc_i, Zminus Zl pc_i))
                nil))
        else
          choose (Zminus (0%Z) pc_i, Zminus Zl pc_i) 
      in
      liftGen2 BNZ gen_offset (elems_ Z0 num));
    (* PSetOff *)
...
Fixpoint gen_by_exec (fuel : nat) (orig_st st: SState) : G SState :=
  let terminate_with_imem (st : SState) :=
    let '(St im m s r pc) := orig_st in
    ret (St (st_imem st) m s r pc) in
  match fuel with
  | O =>
  (* End of fuel, return the original with updated imem *)
      terminate_with_imem st
  | S fuel' =>
      let '(St im m s r pc) := st in
      let '(PAtm pc_i pc_l) := pc in
      match SState_instr_lookup st with
      | Some Nop =>
          (* Generate Instruction *)
          bindGen (ainstrSSNI st) (fun instr =>
          let im' := set_nth instr im (Z.to_nat pc_i) instr in          
          let st' := St im' m s r pc in
          match fstep default_table st' with
          | Some st'' => gen_by_exec fuel' orig_st st''
          | None =>
              terminate_with_imem st'
          end)
      | Some i =>
          (* Already generated instruction, step *)
          match fstep default_table st with
          | Some st'' => gen_by_exec fuel' orig_st st''
          | None =>
              terminate_with_imem st 
...
Definition gen_var_frame (obs: Label) (inf : Info) (f : memframe Atom)
: G frame :=
    ...
    if isHigh lab obs then
      ...
      let genSequence := sequenceGen(map (gen_vary_atom obs inf) data) in
      returnGen (Fr lab data).
...
Definition gen_variation_SState : G (@Variation SState) :=
  ...
  match dfs with
    | (def, _) :: _ =>
      ...
      (* For LLNI, PC starts at 0 *)
      bindGen (ret (PAtm 0 bot)) (fun pc =>
      bindGen (smart_gen inf) (fun regs =>
      ...
      (* Instantiate instructions by execution *)
      let st := St imem m stk regs pc in
      bindGen (gen_by_exec 30 st st) (fun st =>
      (* Instantiate stamps *)
...
#[global] Instance shrink_variation_SState : Shrink (@Variation SState) := {
  shrink := fun v => [::]
}.
(* #[global] Instance show_variation_SState : Show (@Variation SState) := {
  show := fun v => "Variation"
}. *)

Axiom number_of_trials : nat.
Extract Constant number_of_trials => "max_int".

Definition propEENI (t : table) (v : Variation) : option bool :=
...
Definition unwrap_or {A : Type} (o : option A) (a: A) : A :=
  match o with
  | Some a => a
  | None => a
  end.
