...
{| mkPool _ := QueuePool.mkQueuePool tt;
  ...
  sample pool := match QueuePool.pop_front pool with
                 | None => Generate
                 | Some (seed, _) => Mutate seed 0
                 end ;
  best pool := match QueuePool.pop_front pool with
               | None => None
...
Fixpoint toMonad {C : Ctx} {F: Type} (cprop: CProp C F) : ⟦C⟧ -> Checker :=
  ...
  | ForAll A C F name gen mut shr pri cprop' =>
    fun env =>
      forAllShrinkShow (gen env) (shr env) (pri env) (fun a => @toMonad (A · C) F cprop' (a, env))
  | ForAllMaybe A C F name gen mut shr pri cprop' =>
    fun env =>
      forAllShrinkShowMaybe (gen env) (shr env) (pri env) (fun a => @toMonad (A · C) F cprop' (a, env))
  | Predicate C F prop =>
    fun env => 
...
(* Sample1 (targetLoop 1000 example3 (HeapSeedPool.(mkPool) tt)  HillClimbingUtility). *)
